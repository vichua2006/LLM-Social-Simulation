    ...
    {{
      ...
      Payload:{{
        TradePayload:{{
          ...
          Pay:{{
            description: "The type of resource you want to trade with others, only select from one of the [land, food, glory]",
            value: string
          }}
          PayAmount:{{
        ...
        RobPayload:{{
          ...
          RobItem{{
            description: "The type of resource you want to rob from others, only select from one of the [land, food, glory]",
            value: string 
          }}
        }}
    ...
    for i in range(error_correction):
      ...
      if not progress[0]:
        try:
          ...
          pass
    #done_action=str_to_bool(response[1])
    #if done_action:del system.pending_action[done_action]
      assistence[2]=q[2]
      if not progress[1]:
        try:
